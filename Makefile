...
build-local:
	...
	go build -o bin/registry.terraform.io/dmalch/familio/$(VERSION)/$(PLATFORM)/terraform-provider-familio
...
build:
	@echo "${WHITE}=====================${NC}"
	@echo "${YELLOW}Building...${NC}"
	go build -o bin/terraform-provider-familio
	@echo "${YELLOW}Building...${NC} ${GREEN}Done${NC}"
...
docs:
	...
	tfplugindocs generate --provider-name familio
